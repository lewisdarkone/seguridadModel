...
public class ResponseLoggingMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext httpContext, RequestDelegate next)
    {
       ...
       .Information("HTTP {RequestMethod} Request to {RequestPath} by {RequesterEmail} has Estatus Code {StatusCode}.", httpContext.Request.Method, httpContext.Request.Path, string.IsNullOrEmpty(email) ? "Anonymous" : email, httpContext.Response.StatusCode);
        newBody.Seek(0, SeekOrigin.Begin);
