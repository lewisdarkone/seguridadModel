...
using com.softpine.muvany.infrastructure.Extensions;
...
namespace com.softpine.muvany.infrastructure.FileStorage;
...
public class LocalFileStorageService : IFileStorageService
{
    ...
    where T : class
    {
        ...
        if (request.Extension is null || !supportedFileType.GetDescriptionList().Contains(request.Extension.ToLower()))
            throw new InvalidOperationException(ApiConstants.Messages.FileFormatNotSupported);
        if (request.Name is null)
            throw new InvalidOperationException(ApiConstants.Messages.NameIsRequired);
